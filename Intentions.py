...
def get_allowed_moves(tile, prev_move):
    ...
    if tile == '4way':
        return moves
    tile_type, direction = tile.split('/')
    if direction == 'N':
        ok_moves = [3]
...
def get_all_entries(tile):
    ...
    tile_type, direction = tile.split('/')
    if tile_type == 'straight':
        return [direction, cardinals[(cardinals.index(direction) + 2) % 4]]
    elif tile_type == 'curve_left':
        return [cardinals[(cardinals.index(direction) + rotation) % 4] for rotation in [2, 1]]
    elif tile_type == 'curve_right':
        return [cardinals[(cardinals.index(direction) + rotation) % 4] for rotation in [-2, -1]]
    elif tile_type == '3way_left':
        return [cardinals[(cardinals.index(direction) + rotation) % 4] for rotation in [2, 1, 0]]
    elif tile_type == '3way_right':
        return [cardinals[(cardinals.index(direction) + rotation) % 4] for rotation in [-2, -1, 0]]
